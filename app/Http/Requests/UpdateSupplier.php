...
class UpdateSupplier extends FormRequest
{
    ...
    {
        $id = $this->route('supplier'); // Assuming you're using route model binding
        ...
        return [
            's_name' => [
                ...
                'string',
                'max:30',
                // Conditionally apply unique rule only if s_name is modified
                Rule::unique('suppliers', 's_name')->ignore($id)->where(function ($query) {
                    $currentName = $this->input('s_name');
                    $query->where('s_name', '!=', $currentName);
                }),
            ],
            'mobile1' => [
                'required',
    ...
    public function messages()
    {
        return [
            ...
            's_name.unique' => 'The supplier name has already been taken. Please choose a different name.',
            'mobile1.required' => 'Please provide the mobile number (1) for the supplier.',
            'mobile1.size' => 'Mobile number (1) should be exactly 10 digits.',
